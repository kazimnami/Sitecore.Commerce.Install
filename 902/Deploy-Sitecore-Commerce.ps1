...
$params = @{
		...
		CommerceServicesDbName = "$($prefix)_Commerce.SharedEnvironments"
		CommerceServicesGlobalDbName = "$($prefix)_Commerce.Global"		
		...
		SitecoreBizFxServicesContentPath = Resolve-Path -Path "..\Sitecore.BizFX.1*.zip"		
		SitecoreIdentityServerZipPath = Resolve-Path -Path "..\Sitecore.IdentityServer.1.*.zip"
		CommerceEngineCertificatePath = "C:\certificates\$($prefix).commerce.engine.cer"		
		...
		MergeToolFullPath = Resolve-Path -Path "..\assets\MSBuild.Microsoft.VisualStudio.Web.targets.14.0.0.3\tools\VSToolsPath\Web\Microsoft.Web.XmlTransform.dll"
		UserAccount = @{
		...
		BraintreeAccount = @{
			MerchantId = 'rwd84b5k2rck8c7f'
			PublicKey = '747f5tsgkbk9xrk3'
			PrivateKey = '25d3cfa5a9674a8b12e167af80b8607f'
		}
		SitecoreIdentityServerName = "$($prefix).IdentityServer"		
    }
